...
public class NewHDFSServerWizardPage extends WizardPage
{
	...
	private static final Logger logger = Logger
			.getLogger( NewHDFSServerWizardPage.class );
	...
	public void createControl( Composite parent )
	{
		...
		final Button overrideSecurityCheckbox = new Button( securityGroup,
				SWT.CHECK );
		gd = new GridData( );
		gd.horizontalSpan = 2;
		...
		final Text userIdText = new Text( securityGroup,
		...
		groupIdsLabel.setLayoutData(
				new GridData( GridData.VERTICAL_ALIGN_BEGINNING ) );
		final org.eclipse.swt.widgets.List groupsList = new org.eclipse.swt.widgets.List(
				securityGroup, SWT.BORDER );
		...
		if ( userAndGroupIds != null && userAndGroupIds.size( ) > 1 )
			for ( String groupId : userAndGroupIds.subList( 1,
					userAndGroupIds.size( ) ) )
		...
		overrideSecurityCheckbox.addSelectionListener( new SelectionAdapter( ) {
			...
			{
				overrideDefaultSecurity = overrideSecurityCheckbox
						.getSelection( );
	...
	private List<String> getUserAndGroupIds( )
	{
		...
		try
		{
			HDFSClient client = HDFSManager.INSTANCE
					.getClient( hdfsServerLocation );
			List<String> defaultUserAndGroupIds = client
	...
	{
		setPageComplete( getHdfsServerName( ) != null
				&& getHdfsServerLocation( ) != null
				&& ( !overrideDefaultSecurity || userId != null ) );
	}
